// src/lib/branding.ts
// Standalone F9 landing site — brand constants
...
export const BRAND = {
  ...
  // URLs
  // LANDING_URL  = this site (what Monnify sees)
  // PLATFORM_URL = the real app (links in CTAs once it is live)
  DOMAIN:       'f9.ng',
  LANDING_URL:  'https://f9-landing.vercel.app', // swap for custom domain later
  PLATFORM_URL: 'https://f9.ng',
  ...
  RC_NUMBER:  'RC-XXXXXX', // update before submitting to Monnify

  // Marketing copy
  HERO_HEADLINE:    '⚡ Hustle Forward with F9',
  HERO_SUBHEADLINE: "Nigeria's #1 student freelance marketplace",
  CTA_PRIMARY:      'Join the Waitlist',   // Platform not live yet
  ...
  // Open Graph
  OG_IMAGE: '/og-image.png', // 1200x630 — add to /public before deploy
  OG_TYPE:  'website',
...
export const getBrandName = (
  variant: 'short' | 'full' | 'tagline' = 'short'
): string => {
  switch (variant) {
    case 'full':    return BRAND.FULL_NAME
    case 'tagline': return `${BRAND.NAME} — ${BRAND.TAGLINE}`
    default:        return BRAND.SHORT_NAME
  }
}

export const getBrandEmail = (
  type: 'support' | 'legal' | 'press' = 'support'
): string => {
  switch (type) {
